...
### 뉴스 요약
chat gpt를 개발한 샘 알트만이 본인이 개발한 chat gpt에 대한 느낀점과 개발한 프로그램에 대한 부작용에 대하여 설명한 내용이다.
chat gpt는 완전하지 않으며 이를 악용하는 사람들도 있을 것이라고 말하였고, 인간의 일을 대신하는 것에대해서 빠르게 일어날수 있는 일이라고 하였다.
이를 통해 우리가 해야할 일은 기억하는 것이 아닌 추론하는 능력이며, 인가의 창의성은 무한하기에 이 문제를 해결할 수 있도록 인간이 대체할 수 있는 일을 찾을 것이라고 말하였다.
또한 chat gpt는 작업의 대체품이 아닌 도구로 이용될 것이라고 하였다.


### 느낀점

해당 노드를 진행하면서 자주 쓰지 않았던 fork에 대해서 공부를 하게 되었고 평소 의문이었던 fork와 clone에 대해서도 알게 되었다
또한 평소 모호하던 pull과 clone에 대한 차이에 대해서도 알게 되었다.
이번 노드를 진행함에 따라 그동안 자주 사용하던 git에서의 명령어도 내가 정확하게 모르는 내용도 있을수 있다라는 것을 알게되었고 앞으로는 이부분을 참고하여 진행하도록 노력할 생각이다.
...
샘 알트만 본인이 스스로 chat-gpt를 발전시키면서 이를 악용할 부분에 대해 또 걱정하는 딜레마에 대한 부분이다.   
현재 chat-gpt의 성능은 특정 부분에 있어서는 이미 인간의 능력을 뛰어넘었다고 말하는 부분도 있으며   
현재 chat-gpt의 언어는 영어로 하는게 가장 인식 성능이 높지만 다른 언어도 개선하겠다는 뜻을 밝혔다.   
더 나아가 글자만을 이해하는것이 아닌 라벨을 번역, 지도를 읽는 등 다양한 분야에서 이해도가 높다고 한다.

### 느낀 점 

개발자 스스로도 인공지능의 성능에 대해 걱정을 하고 있다는 사실이 신기하다. 그 걱정하는 부분이 성능이 모자라다 라는것이 아닌 성능이 너무 뛰어나 위험하다 라고 표현한것 자체가 신선한 충격이며 영화에서만 봐오던 AI에 지배당하는 세상이 올수도 있겠다 라는 생각을 하며 그렇기 때문에 AI 분야에 대해 확실히 공부할 필요가 있다는 점을 다시 한번 상기시켜주는 기사인것 같다.
